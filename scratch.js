...
const addItems = function(dataGet){
    ...
    for(var k = 0; k < dataGet.length; k++){
      for(var j = 0; j < dataGet[k].reviews.length; j++){
        ...
        $(".root").append(`<div>${dataGet[k].reviews[j].rating}</div><button itemId="${dataGet[k]._id}" ratingid="${dataGet[k].reviews[j]._id}">Delete Rating</button>`);
        ...
        const $button1 = $(`<button commentid="${dataGet[k].reviews[j]._id}">`).attr('dataGet-commentid', dataGet[k].reviews[j]._id).text('Delete Review').on("click", deleteComment);
        $(".root").append($button1)
...
// for(var k = 0; k < dataGet.length; k++){
//     for(var j = 0; j < dataGet[k].reviews.length; j++){
//       $(".root").append(`<div>${dataGet[k].reviews[j].comment}</div><button itemId="${dataGet[k]._id}" commentId="${dataGet[k].reviews[j]._id}">Delete Review</button>`);
